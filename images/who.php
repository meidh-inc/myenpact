		...
		if (mysql_affected_rows() == 1) {
		    ...
		    //$message = "<center><p style=\"color:blue;\">The page was successfully updated.</p></center>";
                    ...
                    if(!$_FILES['logo_edit'] || empty ($_FILES['logo_edit']) || !is_array($_FILES['logo_edit'])) {
                        ...
                        $logo_errors[] = "No file was uploaded.";
                        return false;
                    } elseif ($_FILES['logo_edit']['error'] != 0) {
                        // error: report what PHP says went wrong
                        $logo_errors[] = $upload_errors[$_FILES['logo_edit']['error']];
                        return false;
                    } else {
                    ...
                    if(!empty($logo_errors)) { return false; } 

                    // Can't save without filename and temp location
                    if(empty($filename) || empty($temp_path)) {
                        $logo_errors[] = "The file location was not available.";
                        return false;
                    }
                    ...
                    if (file_exists($target_path)) {
                        $logo_errors[] = "The file {$filename} already exists.";
                        return false;
                    }
                    ...
                    if(move_uploaded_file($temp_path, $target_path)) {
                        ...
                        // File was not moved.
                        $logo_errors[] = "The file upload failed, possibly due to incorrect permissions on the upload folder.";
                        return false;
                    }
		    redirect_to("who.php");
		} else {
...
<table border='0' style="width:750px;">
	...
	<td style="height:50px;">
	    ...
	    <br>
            ...
            <input type="file" name ="logo_edit" maxlength="100" style="width:250px;"
                value="<?php echo htmlentities($logo_edit); ?>" />
	</td>
...
<table border='0' style="width:850px;background-image:url('images/texture2.jpg');padding:30px;margin-top:5px;">
    ...
    <tr><td colspan='3' style="height:1px;border-top:solid gray thin;padding-bottom:20px;"></td></tr>
    <tr>
	<td>
	    FROM:
	</td>
	<td style="height:50px;width:550px;padding-left:20px;">
	    <table>
                <tr>
                    <td><?php echo htmlentities($from_email); ?>
                    <br>
                    <?php if($organization == ""){$organization = "Organization not listed";} echo htmlentities($organization); ?></td>
                    <td>
                        <?php echo $_FILES['logo_edit']['name']; ?>
                        <?php if($logo == "") {$logo = "No image chosen"; } echo "<img src=\"images/logos/" . $logo . "\" name=\"logo\" height=\"100\">"; ?></td>
                </tr>
            </table>
	</td>
	<td style="">
    ...
    <tr><td colspan='3' style="height:1px;border-bottom:solid gray thin;padding-top:20px;"></td></tr>
    <tr>
    ...
    <tr><td colspan='3' style="height:1px;border-top:solid gray thin;"></td></tr>
