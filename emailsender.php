...
while($array_recipients = mysql_fetch_array($get_recipients)){  // looping through array of recipients
    ...
    $find_question = mysql_query("SELECT * FROM results WHERE
					...
					ORDER BY id DESC");
    $find_question_array = array();
    
    $index = 0;

    while($row = mysql_fetch_assoc($find_question)) {
        $find_question_array[$index] = $row;
        $index++;
    }
    
    while($array_send_question = mysql_fetch_array($get_send_question, MYSQL_ASSOC)){
	
	//find question in results within $count_send_questions rows

        if(in_array_r($array_send_question['id'],$find_question_array)){
	    $send_question_id = "";
	} else {
    ...
    if($send_question_id != "") {
        ...
        $body = $intro . "<br><h3>" . $send_question . "</h3><br /><br />Please click one of the links below to answer:
                <ul>";
        ...
        for($loop=0; $loop<$num_answers; $loop++){
            ...
            $body .= "<li style=\"margin-left:0px;padding-left:0px;\"><a href=\"http://myenpact.com/emailreceiver.php?q=" . $send_question_id . "&amp;a=" . $answer_no[$loop] . "&amp;rid=" . $recipient_id . "&amp;id=" . $result_id . "\">" . $answer[$loop] . "</a></li>";
        }
        ...
        $body .="</ul>
                ...
                <br /><br />I would not like to receive additional survey emails: <a href=\"http://myenpact.com/emailreceiver.php?uid=" . $recipient_id . "&did=" . $result_id . "\">unsubscribe</a>.
                </font>";
        ...
        $mail->AddReplyTo("superjars@gmail.com","Jared Bearss");
        ...
        $sql_d = "DELETE 
            ...
            AND recipient_id = '{$recipient_id}'
            AND send_id = {$send}";
        $delete_result = mysql_query($sql_d);
    }
}
