...
if(isset($_GET['kid'])){ //user might be logging in for the first time
    $userid = $_GET['uid'];
    $signup_key = $_GET['kid'];
    $check_verification = mysql_query("SELECT verified 
								FROM users 
								WHERE id = '{$userid}' ");
	...
	if($array_verification['verified'] != 'yes'){ // not yet verified, perform verification
		if($array_verification['verified'] == $signup_key){  // account verified!
	    	$update_verified = mysql_query("UPDATE users 
										SET verified = 'yes' 
										WHERE id = '{$userid}' ");
		}
...
if (isset($_POST['login'])) { // The user clicked 'login'
    ...
    $errors_log = array(); // create array to place any errors
    $required_fields = array('email_log', 'password_log'); // perform validations on the form data
    ...
    $email_log = trim(mysql_prep($_POST['email_log'])); // get the content from the variables
    $password_log = trim(mysql_prep($_POST['password_log']));
    $hashed_password = sha1($password_log); // translate the entered password to hashed
    if ( empty($errors_log) ) { // no errors detected so far, continue
	    
        $query_login = "SELECT id, email, verified 
                FROM users WHERE email = '{$email_log}'
                AND hashed_password = '{$hashed_password}' 
                LIMIT 1"; // Check database to see if username and the hashed password exist there.
        $result_login = mysql_query($query_login);
        confirm_query($result_login);
        $found_user = mysql_fetch_array($result_login);

		
	    if ($found_user['verified'] == 'yes'){ // Pass or not pass
			if (mysql_num_rows($result_login) == 1) {  // Success: username/password authenticated and only 1 match
				// set session id's
...
elseif (isset($_POST['signup'])) {  // user clicked 'signup'
	...
	$required_fields = array('email_sign', 'confirm_email', 'password_sign', 'confirm_password', 'terms' ); // perform validations on the form data
	$errors_sign = array_merge($errors_sign, check_required_fields($required_fields, $_POST));
	$fields_with_lengths = array('email_sign' => 250, 'confirm_email' => 250, 'password_sign' => 30, 'confirm_password' => 30);
	...
	elseif(mysql_num_rows($query_verify_email) != 0){  // email is already in the system, deliver error message
	    ...
	    if ( empty($errors_sign) ) {  // no errors found, sign the user up
		    $conf_code = substr(md5(uniqid(rand(), true)), -16, 16); // Generate the confirmation code to be emailed
		    $timestamp_sign = date("Y-m-d h:i:s a"); // Get timestamp for signup
		    $key = md5(uniqid(rand(), TRUE));
			$query_sign = "INSERT INTO users ( email, verified, hashed_password, signup_date) 
						VALUES ( '{$email_sign}', '{$key}', '{$hashed_password}', '{$timestamp_sign}')"; // Enter user info into DB
			...
			if ($result_sign) { //user was successfully created, send them a confirmation email
				...
				if(!$mail->Send()) {  //Email Failed
					...
					redirect_to('index.php'); // echo "Message sent! to: Someone at ".$email_to." \n";
				}
...
<table border='0' style="width:850px;background-image:url('images/texture2.jpg');margin:0px;margin-top:5px;padding:0px;">
    ...
    <tr>
        <td colspan='2' style="height:200px;text-align:left;border-top:gray thin solid;margin:0px;padding:0px;">
            <div id="wrapper"><div id="banner" style="text-align:center;">Website<br>Under Construction</div><img src="images/index_header.png" alt="index_header" /></div>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <tr>
            <td style="height:150px;width:33%;text-align:center;padding:15px;padding-top:0px;margin-bottom:70px;border-right:2px solid #F4911E;">
                <br>myENpact users utilize a unique one-question-at-a-time survey method to gather powerful long-term data. We then store and display this data to you in a meaningful way.
            </td>
            <td style="height:150px;width:33%;text-align:center;padding:15px;padding-top:0px;margin-bottom:70px;border-right:2px solid #F4911E;">
                <br>These pages are currently being remade in order to provide greater stability, more options and tighter security. Check back often to see what changes are being made.
            </td>
            <td style="height:150px;width:33%;text-align:center;padding:15px;padding-top:0px;margin-bottom:70px;">
                <br>If you are interested in setting up an account and starting to use this product, contact our <a href="mailto:webmaster@myenpact.com" style="color:black;">webmaster</a>.
            </td>
        </tr>
        </table>
        </td>
